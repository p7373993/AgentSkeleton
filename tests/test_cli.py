...
def test_show_run_omits_mixed_timezone_run_duration(monkeypatch, tmp_path) -> None:
    monkeypatch.chdir(tmp_path)
    log_dir = tmp_path / "runs" / "20260511"
    log_dir.mkdir(parents=True)
    log_path = log_dir / "run-1.jsonl"
...
def test_show_run_omits_empty_model_retry_details(
    monkeypatch,
    tmp_path,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    assert result.exit_code == 0
    assert "Model retries: 1" in result.stdout
    assert "Last model retry:" not in result.stdout
...
def test_list_runs_prints_run_event_timestamps(monkeypatch, tmp_path) -> None:
    monkeypatch.chdir(tmp_path)
    log_dir = tmp_path / "runs" / "20260511"
    log_dir.mkdir(parents=True)
    (log_dir / "run-1.jsonl").write_text(
