...
def test_llm_client_avoids_truthiness_checks_for_output_text(tmp_path) -> None:
    class ExplodingOutputText(str):
        def __len__(self) -> int:
            raise RuntimeError("output text length unavailable")

    response = SimpleNamespace(
        id="resp-1",
        output_text=ExplodingOutputText("done"),
        output=[],
    )
...
def test_llm_client_rejects_uninspectable_response_output(tmp_path) -> None:
    class ExplodingOutput(list):
        def __iter__(self):
            raise RuntimeError("output iteration unavailable")

    response = SimpleNamespace(
        id="resp-1",
        output_text="",
        output=ExplodingOutput([{"type": "message"}]),
    )
    state = RunState(run_id="run-1", workspace=tmp_path, goal="hello")
...
def test_llm_client_rejects_uninspectable_response_output_items(tmp_path) -> None:
    class ExplodingOutputItem:
        def __getattr__(self, _name):
            raise RuntimeError("output item unavailable")

    response = SimpleNamespace(
        id="resp-1",
        output_text="",
        output=[ExplodingOutputItem()],
    )
    state = RunState(run_id="run-1", workspace=tmp_path, goal="hello")

    with pytest.raises(
        LLMResponseError,
        match="Response output item could not be inspected",
    ):
        LLMClient(
            RunConfig(workspace=tmp_path),
            client=FakeClient(response),
        ...
        output=[
            {
                "type": "message",
                "role": "assistant",
                "content": [{"type": "output_text", "text": LengthlessText("done")}],
            }
        ],
    )
    state = RunState(run_id="run-1", workspace=tmp_path, goal="finish")
...
def test_llm_client_serializes_uninspectable_object_response_context_item_types(
    tmp_path,
) -> None:
    response = SimpleNamespace(
        id="resp-1",
...
def test_llm_client_skips_uninspectable_object_context_fields(
    ...
    class OutputItemWithExplodingContent:
        type = "message"
        role = "assistant"
    ...
    response = SimpleNamespace(
        id="resp-1",
        output_text="done",
        output=[OutputItemWithExplodingContent()],
    )
    state = RunState(run_id="run-1", workspace=tmp_path, goal="finish")
...
def test_llm_client_normalizes_uninspectable_transcript_roles(tmp_path) -> None:
    response = SimpleNamespace(id="resp-1", output_text="done", output=[])
    fake_client = FakeClient(response)
    state = RunState(
        run_id="run-1",
        ...
        conversation=[
            ConversationMessage(
                role=UninspectableString("assistant"),
